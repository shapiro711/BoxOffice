...
final class NetworkManager {
    ...
    func request(apiModel: APIable, completion: @escaping URLSessionResult) {
        ...
        request.httpBody = createDataBody(parameter: apiModel.param, contentType: apiModel.contentType, mediaFile: apiModel.mediaFile)
...
extension NetworkManager {
    ...
    private func createDataBody(parameter: [String: String?]?, contentType: ContentType, mediaFile: [Media]?) -> Data? {
        var body = Data()
        let lineBreak = "\r\n"
        ...
        if let modelParameter = parameter {
            if contentType == .multiPartForm {
                ...
                guard let modelImages = mediaFile else {
                ...
                if let data = ParssingManager.encodingModel(model: parameter){
                    body = data
                }
            }
        } else {
